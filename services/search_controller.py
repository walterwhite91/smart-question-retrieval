...
class SearchController:
    """Dataset cache plus filter/search operations shared by UI layers."""
    ...
    def get_chapter_counts(self, semester: int | None, subject: str) -> list[dict[str, Any]]:
        rows = self._by_subject(self._by_semester(semester), subject)
        counts: Counter[str] = Counter()
        for row in rows:
            for chapter in row.get("chapter", []):
                if isinstance(chapter, str):
                    counts[chapter] += 1
        return [
            {"name": chapter, "count": counts[chapter]}
